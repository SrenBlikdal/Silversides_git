...
## WGBS analysis of 68 Atlantic silversides

### Read WGBS data mappped to selection-group adjusted reference genomes
The CpG reports are imported and combined into a BS object (n (number of samples) x m (number of sites) matrix of methylation values). 
...
We restrict the analysis to the main chromosomes (Mme_chr01-Mme_chr24) and ignore the unanchored contigs.
...
We import the metadata from S1 and check that the SampleID from the metadata match the sampleNames in the bs object and add the metadata to the BS object.
...
From the lcWGS we have the position and allele frequency (KnownEM) for SNPs from ANGSD for each selction group which we import as GRanges objects
...
Which we use to filter out polymophic sites (knownEM <= 0.99) from the BS object from each selection group and combine it into a new BS object.
...
After filtering out polymorphic sites, we filter for sites with a coverage of at least 2 in >= 49 samples and <=19) >= 49 samples (see sub analysis for details).  
...
### Small fish 
TO DO: change the shape of the points to reflect tissue type. 
A lot of this code is duplicate so consider making a function. 
...
p_s<-ggplot(pca_plot, aes(x=PC1, y=PC2, color=group, fill=group)) +
  geom_point(shape=24)  +
...
### Smoothing the data
Before we start the DMR analysis we smooth the data using the BSmooth function. 
...
###Small fish
We identify DMRs among the small fish using the BSmooth.fstat function. We use the group as a factor in the design matrix and the contrasts are defined as the difference between the groups. We use the makeContrasts function to define the contrasts. We use the dmrfinder function to identify DMRs and filter for regions with at least 5 CpGs. We use permutation testing to estimate the FWER and filter for regions with a FWER < 50 after 1000 permutations. Since the permutations are time-consuming we save the results to an RDS file to read back in later and set the seed to ensure reproducibility.
...
We remove the R1 selection group and do the same for the medium fish. 
...
### Large fish
We do the same for the large size class. 
...
For plotting we do the same analysis without permutations for all size classes combined
...
p<-ggplot(SMP, aes(x=BPcum, y=areaStat)) +
  ...
  geom_point(data=SMP_sig_s,aes(x=BPcum, y=areaStat),alpha=1, size=5, stroke=1, shape=0, color="blue")  +
  geom_point(data=SMP_sig_m, aes(x=BPcum, y=areaStat),alpha=1, size=6, stroke=1, shape=1, color="blue")  +
  geom_point(data=SMP_sig_l,aes(x=BPcum, y=areaStat),alpha=1, size=4, stroke=1, shape=2, color="blue")  +
...
P_DMRgenes<-left_join(P_DMRs,GO,by =join_by("GeneID"=="GeneID")) %>% filter(`e-Value`<1e-7) 
...
proms<-promoters(GenomicFeatures::genes(txdb), upstream = 1500, downstream = 1500)
ol<-findOverlaps(all_group_regions,proms, type="any", select="all")
p <- proms[subjectHits(ol),] %>% as_tibble()
...
GO_dmrs_groups<-tibble(as_tibble(enr@result)) %>% filter(Count>1 & p.adjust<0.05) %>% arrange(p.adjust) %>% dplyr::select(Description, GeneRatio, BgRatio, pvalue, p.adjust, geneID)%>% separate(Description, c("Cat","Description"), sep=":") %>% arrange(desc(as.character(Cat)), p.adjust)
clipr::write_clip(GO_dmrs_groups)
```
...
my_colour = list(group = c(D1Gen5= "#ffc425", D2Gen5="darkorange", R1Gen5="darkgreen", U1Gen5="skyblue", U2Gen5= "darkblue"),size = c(S = "#ddfff7", M = "#83c5be", L = "#006d77"))
...
plotManyRegions(bs.filtered.plot, group_dmrs_merged[c(21,61,82),],addPoints = F, extend=1000, col = bs.filtered.plot$cols, lty = bs.filtered.plot$linet)
plotManyRegions(bs.filtered.plot, group_dmrs_merged[c(40,54,71),],addPoints = F, extend=1000, col = bs.filtered.plot$cols, lty = bs.filtered.plot$linet)
```
...
bs.filtered.fit.tstat <- BSmooth.tstat(BSseq=bs.filtered_D2_invH_f,  
                                    ...
                                    mc.cores =1,
                                    verbose = TRUE)
dmrs_D2_inv_all<-bsseq::dmrFinder(bs.filtered.fit.tstat, cutoff = c(-0.6, 0.6), verbose = F) %>% filter(n>=5)
dmrs_D2_inv_sig <- bsseq::dmrFinder(bs.filtered.fit.tstat, cutoff = c(-4.6, 4.6), verbose = F) %>% filter(n>=5)
dmrs_D2_inv_sig %>% group_by(chr) %>% summarise(n=n())
```

```{r}
regions<-as(dmrs_D2_inv_all, "GRanges")
positions<-ranges(regions)
pos<-data.frame(chr=seqnames(regions),positions, areaStat=regions$areaStat) %>% filter(str_starts(chr, "Mme"))
...
```{r}
dmrs_D2_chr24inv <- dmrs_D2_inv_sig %>% arrange(as.character(chr), start)
...
pheat<-pheatmap(as.matrix(meth), cluster_rows = F, annotation_colors=my_colour, annotation_col= sample_col)
```
### May be replaced with genotypic data
```{r}
...
```{r}
all_regions_D2<-dmrs_D2_inv_all %>% as(.,"GRanges")
proms<-promoters(GenomicFeatures::genes(txdb), upstream = 1500, downstream = 1500)
...
GO_dmrs_D2_chr24inv<-tibble(as_tibble(enr@result)) %>% filter(Count>1 & p.adjust<0.05) %>% arrange(p.adjust) %>% dplyr::select(Description, GeneRatio, BgRatio, pvalue, p.adjust, geneID)%>% separate(Description, c("Cat","Description"), sep=":") %>% arrange(desc(as.character(Cat)), p.adjust)
clipr::write_clip(GO_dmrs_D2_chr24inv)
```

```{r}
dmrs_D2_chr24inv_table<-P_DMRgenes_sig %>% filter(`e-Value`<1e-7) %>% dplyr::select(DMR, GeneID,Description) %>% unique(.) %>% left_join(dmrs_D2_chr24inv,.,by="DMR") %>% dplyr::select(DMR	,chromosome=chr, start, end, n, width, invdensity, areaStat, maxStat, meanDiff, group1.mean, group2.mean, GeneID, Description)
